...
const books=fs.readdirSync(booksRoot,{withFileTypes:true}).filter(d=>d.isDirectory()).map(d=>{
 ...
 if(book.id!==d.name)throw new Error("ID incompatível com diretório: "+d.name);
 const chapters=JSON.parse(fs.readFileSync(path.join(dir,book.chapters.path),"utf8")).chapters||[];
 return {...book,bookId:book.id,chapterList:library.chapters(chapters).map(c=>({chapterId:c.chapterId,number:c.number})),chaptersUrl:library.asset(book,book.chapters.path),coverLegacyUrl:book.cover.legacyUrl,cover:"livros/"+book.id+"/"+book.cover.path,url:"reader.html?book="+encodeURIComponent(book.id),legacyUrl:book.legacy?.url,currentChapter:book.chapters.currentPublished,chapterCount:chapters.length};
